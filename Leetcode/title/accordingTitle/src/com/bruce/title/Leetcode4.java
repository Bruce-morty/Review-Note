...
public class Leetcode4 {
    ...
    private static int middleElement(int[] arr1, int[] arr2, int middle) {
        ...
        while (true) {
            ...
            if (pivot1 <= pivot2) {
                // arr1的前面元素要被排除
                // middle = middle - (最新的元素索引 - 被排除时的开始索引 + 1） = 长度
    ...
    public static double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        int end1 = len1 - 1;
        int end2 = len2 - 1;
        // index代表的不是索引，指的是第K大的元素，即中位数
        // e.g. 长度为3, 3 / 2 = 1; 找第一大的数，错了， (3 + 1) / 2 = 2 找第二大的数就是中位数
        // 长度为4, (4 + 1) /2 = 2 (4 + 2) / 2 = 3，计算2 + 3位置的平均值
        int middleIndex1 = (len1 + len2) / 2;
        int middleIndex2 = (len1 + len2) / 2 + 1;
